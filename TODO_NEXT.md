...
## Active Sprint: GUI Recovery And Core Workflow Stabilization
- Current lane for the next pass: GUI bugfix/workflow stabilization only.
- Current automated status: Debug MSBuild, `OurIPH.Tests.exe`, WPF `--smoke-test`, and no-leftover-process check passed on 2026-05-17 after the readable-status/mojibake fix.
- Current manual status: interactive WPF checklist has not been run in this Codex session because real interactive UI viewing/clicking is unavailable here.
- Priority order:
  1. Run the automated baseline: close workspace `OurIPH.exe`, build Debug, run `OurIPH.Tests.exe`, run WPF `--smoke-test`, and verify no hidden `OurIPH.exe` remains.
  2. Run or prepare the manual UI pass from `UI_SMOKE_CHECKLIST.md`.
  3. Fix only concrete GUI/workflow bugs found by the checklist or by direct inspection of the matching WPF event handlers.
  4. Preserve calculation formulas unless a concrete confirmed GUI/workflow bug requires a minimal tested logic fix.
  5. Do not continue `CalculateBlueprintEstimate` extraction in this sprint.
  6. Do not continue full `ConvertToOre.vb` LP numeric parity in this sprint.
  7. Do not add new features or columns unless required to repair the main workflow.
- If interactive WPF access is unavailable, do not substitute unrelated refactor/parity work. Run the automated baseline, inspect the highest-risk GUI workflow handlers, document the blocker, and provide exact manual verification steps for the user.
- Last GUI fix: repaired mojibake in status bar/workflow messages and status-service output. Manual verification should confirm readable Russian text in database status, Blueprint search/list status, queue clear/remove status, project stock/import/build-buy statuses, missing-facility message boxes, and page subtitles.
...
- First manual micro-check after this pass: launch the app, switch through Blueprints/Analysis/Prices/Stations/Build Queue/Current Projects, search `rifter`, press Clear List, add one item to queue, clear queue, create/recalculate a small project if possible, and confirm no `Р...`/`С...` mojibake appears in status lines or message boxes.
...
- Continue splitting `MainWindow.xaml.cs` into view models/application services only in a future refactor/extraction lane, not during the current GUI recovery sprint. Do not continue `CalculateBlueprintEstimate` extraction until the remaining local parity tasks below are stronger and a refactor lane is explicitly selected.
- ConvertToOre/mineral fallback parity is mapped for safe gate/classification/fallback cases, and the current greedy ore planner is extracted/tested in `OrePlanningService`. Full `ConvertToOre.vb GetOresfromMinerals` LP numeric parity is backlog/provisional, not the active next task.
...
## Completed Historical Checklists
These checklists are retained as project memory only. They are not the active lane unless a future prompt explicitly reopens one of them.

- Completed checklist - Ore planning service boundary:
...
- Completed checklist - Facilities / Station preset persistence and UX:
  ...
  - [x] Verify normal Debug build, console tests, WPF smoke, and no leftover `OurIPH.exe`.
- Completed checklist - Manual UI smoke preparation:
  ...
  - [x] Verify normal Debug build, console tests, WPF smoke, and no leftover `OurIPH.exe`.
- Completed checklist - Project estimate display/summary mapping extraction:
  ...
  - [x] Verify normal Debug build, console tests, WPF smoke, and no leftover `OurIPH.exe`.
- Completed checklist - Estimate summary/display mapping extraction:
  ...
  - [x] Verify normal Debug build, console tests, WPF smoke, and no leftover `OurIPH.exe`.
- Completed checklist - CalculateBlueprintEstimate safe orchestration boundary:
...
- Completed checklist - Surplus / excess / sellback legacy parity:
...
- Completed checklist - Recursive component-cost legacy parity harness:
...
- Completed checklist - Numeric legacy parity material cost and fees:
...
- Completed checklist - Numeric legacy parity foundation:
...
- Completed checklist - Legacy/database-backed golden fixture foundation:
...
- Completed checklist - Delegate-backed material traversal extraction:
  ...
  - [x] Verify normal Debug build, console tests, and WPF smoke after the extraction.
- Completed checklist - CalculateBlueprintEstimate migration foundation:
...
- Completed checklist - Recursive estimate orchestration extraction:
  ...
  - [x] Verify normal Debug build, console tests, and WPF smoke after the refactor.
- Completed checklist - Estimate orchestration service extraction:
  ...
  - [x] Add regression tests for the new services.
  - [x] Verify normal Debug build, console tests, and WPF smoke after the refactor.
- Completed checklist - Local audit refresh after efficiency/import/SVR/store passes:
...
- Completed checklist - Contract price store round-trip tests:
...
- Completed checklist - Market cache store round-trip tests:
  ...
  - [x] Verify normal Debug build, console tests, and WPF smoke after the store/test changes.
- Completed checklist - Sales volume ratio service extraction:
  ...
  - [x] Verify normal Debug build, console tests, and WPF smoke after the refactor.
- Completed checklist - Blueprint efficiency service extraction:
  ...
  - [x] Verify normal Debug build, console tests, and WPF smoke after the refactor.
- Completed checklist - Project material clipboard import extraction:
  ...
  - [x] Verify normal Debug build, console tests, and WPF smoke after the refactor.
- Completed checklist - Local audit refresh after queue/project/production extractions:
  ...
  - [x] Verify normal Debug build, console tests, and WPF smoke after documentation updates.
- Completed checklist - Blueprint production type service extraction:
  ...
  - [x] Verify normal Debug build, console tests, and WPF smoke after the refactor.
- Completed checklist - Project material used-by list extraction:
  ...
  - [x] Verify normal Debug build, console tests, and WPF smoke after the refactor.
- Completed checklist - Build project decision service extraction:
  ...
  - [x] Verify normal Debug build, console tests, and WPF smoke after the refactor.
- Completed checklist - Build queue profitable candidate extraction:
  ...
  - [x] Verify normal Debug build, console tests, and WPF smoke after the refactor.
- Completed checklist - Build queue merge service extraction:
  ...
  - [x] Verify normal Debug build, console tests, and WPF smoke after the refactor.
- Completed checklist - Local audit refresh:
  ...
  - [x] Re-scan active source for TODO/FIXME/HACK/XXX markers.
  - [x] Verify normal Debug build, console tests, and WPF smoke after documentation updates.
- Completed checklist - Sales fee service extraction:
  ...
  - [x] Verify normal Debug build, console tests, and WPF smoke after the refactor.
- Completed checklist - Market price selection service extraction:
  ...
  - [x] Verify normal Debug build, console tests, and WPF smoke after the refactor.
- Completed checklist - Market history cache service extraction:
  ...
  - [x] Verify normal Debug build, console tests, and WPF smoke after the refactor.
- Completed checklist - Market price cache service extraction:
  ...
  - [x] Verify normal Debug build, console tests, and WPF smoke after the refactor.
- Completed checklist - Settings path consolidation:
...
- Completed checklist - UI settings store extraction:
  ...
  - [x] Verify normal Debug build, console tests, and WPF smoke after the refactor.
- Completed checklist - Local technical debt audit and layout docs:
  ...
  - [x] Verify normal Debug build, console tests, and WPF smoke after documentation updates.
- Completed checklist - Price update status extraction:
  ...
  - [x] Verify normal Debug build, console tests, and WPF smoke after the refactor.
- Completed checklist - Numeric input validation extraction:
  ...
  - [x] Verify normal Debug build, console tests, and WPF smoke after the refactor.
- Completed checklist - Blueprint build profile extraction:
  ...
  - [x] Verify normal Debug build, console tests, and WPF smoke after the refactor.
- Completed checklist - Blueprint type filter extraction:
  ...
  - [x] Verify normal Debug build, console tests, and WPF smoke after the refactor.
- Completed checklist - Blueprint list filter clarity/testability:
...
- ConvertToOre/mineral fallback parity is now mapped for the safe gate/classification/fallback cases, and the current greedy ore planner is extracted into `OrePlanningService`. Full `ConvertToOre.vb GetOresfromMinerals` LP parity remains provisional backlog work and is not part of the active GUI recovery sprint.
