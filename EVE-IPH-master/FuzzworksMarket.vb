...
Public Class FuzzworksMarket
    ...
    Private Const MaxTypesPerQuery As Integer = 30
    ...
    Public Function GetPrices(ByVal TypeIDList As List(Of PriceItem)) As List(Of FuzzworksMarketPrice)
        ...
        For Each Item In TypeIDList
            ...
            ' Keep Fuzzworks requests small enough to avoid timeout-heavy bulk updates.
            If Len(FuzzworksMarketMainQuery & FindPQ.RegionOrSystemHeader & FindPQ.PriceLocation & FindPQ.ItemList) > 1200 Or FindPQ.Items.Count >= MaxTypesPerQuery Then
        ...
        For Each Record In FinalQueryList
            Try
                ...
                Dim Output As String = GetJSONFile(FuzzworksMarketMainQuery & Record.RegionOrSystemHeader & Record.PriceLocation & "&" & Record.ItemList.Substring(0, Len(Record.ItemList) - 1), "Fuzzwork Market Prices", True)

                If Output = "" Then
                    ErrorData.Description = "Fuzzworks request timed out or returned no data. Try a smaller item selection or wait and try again."
                    ErrorData.Number = -1
                    Return Nothing
                End If
