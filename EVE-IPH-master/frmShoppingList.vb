...
Public Class frmShoppingList
    ...
    Private WithEvents btnAddToProject As New Button
    ...
    Public Sub New()
        ...
        lstBuild.Columns.Add("Циклы", 70, HorizontalAlignment.Right)
        lstBuild.Columns.Add("Будет", 80, HorizontalAlignment.Right)
        lstBuild.Columns.Add("Остаток", 70, HorizontalAlignment.Right)
    ...
    Private Sub ConfigureProjectButtons()
        Me.AutoSize = False
        Me.Width = 1360
        Me.Height = 760
        Me.ClientSize = New Size(1344, 720)
        Me.FormBorderStyle = FormBorderStyle.FixedSingle

        lstBuy.Width = 1328
        lblItemstoBuy.Width = 1328
        lstItems.Width = 870
        lblItemstoBuild.Width = 870
        lstBuild.Left = 890
        lstBuild.Width = 438
        lblComponentstoBuild.Left = 890
        lblComponentstoBuild.Width = 438

        btnCopy.Location = New Point(7, 543)
        btnSaveListToFile.Location = New Point(7, 579)
        btnLoadListFromFile.Location = New Point(7, 615)
        btnClear.Location = New Point(7, 651)
        btnClose.Location = New Point(107, 651)
        lblTotalItemsInList.Location = New Point(107, 543)
        btnSaveSettings.Location = New Point(107, 615)

        btnAddToProject.Location = New Point(207, 543)
        btnAddToProject.Size = New Size(132, 32)
        btnAddToProject.Text = "Добавить в проект"
        btnAddToProject.UseVisualStyleBackColor = True
        If Not Me.Controls.Contains(btnAddToProject) Then
            Me.Controls.Add(btnAddToProject)
        End If
        btnAddToProject.BringToFront()

        gbOptions.Location = New Point(350, 543)
        gbUpdateList.Location = New Point(680, 543)
        chkRebuildItemsfromList.Location = New Point(680, 667)

        lblTC.Location = New Point(1048, 549)
        lblTotalCost.Location = New Point(1145, 547)
        lblTIC.Location = New Point(1018, 568)
        lblTotalInventionCost.Location = New Point(1145, 566)
        lblTCC.Location = New Point(1038, 587)
        lblTotalCopyCost.Location = New Point(1145, 585)
        lblTV.Location = New Point(1033, 606)
        lblTotalVolume.Location = New Point(1145, 604)
        lblTotalBuiltVolume1.Location = New Point(988, 625)
        lblTotalBuiltVolume.Location = New Point(1145, 623)
        lblAvgIPH1.Location = New Point(1018, 644)
        lblAvgIPH.Location = New Point(1145, 642)
        lblTotalProfit1.Location = New Point(1008, 663)
        lblTotalProfit.Location = New Point(1145, 661)

        lblTotalCost.Width = 183
        lblTotalInventionCost.Width = 183
        lblTotalCopyCost.Width = 183
        lblTotalVolume.Width = 183
        lblTotalBuiltVolume.Width = 183
        lblAvgIPH.Width = 183
        lblTotalProfit.Width = 183
    End Sub

    Private Sub btnAddToProject_Click(sender As Object, e As EventArgs) Handles btnAddToProject.Click
        If TotalShoppingList.GetNumShoppingItems = 0 Then
            MsgBox("Shopping list пуст. Сначала добавьте расчет в список.", vbInformation, Application.ProductName)
            Return
        End If

        Dim ProjectName As String = InputBox("Название проекта:", "Добавить в текущие проекты", "Проект " & Now.ToString("dd.MM.yyyy HH:mm"))
        If Trim(ProjectName) = "" Then
            Return
        End If

        CurrentProjectStore.AddProjectFromShoppingList(ProjectName, TotalShoppingList)
        MsgBox("Проект добавлен.", vbInformation, Application.ProductName)

        Dim ProjectsForm As New frmCurrentProjects
        ProjectsForm.Show()
    End Sub
        ...
        If Not IsNothing(BuildItems) Then
            If Not IsNothing(BuildItems.GetBuiltItemList) Then
                For i = 0 To BuildItems.GetBuiltItemList.Count - 1
                    Dim BuildCycles As Long = 0
                    Dim ProducedQuantity As Long = 0
                    Dim ExcessQuantity As Long = 0

                    If BuildItems.GetBuiltItemList(i).PortionSize > 0 Then
                        BuildCycles = CLng(Math.Ceiling(BuildItems.GetBuiltItemList(i).ItemQuantity / BuildItems.GetBuiltItemList(i).PortionSize))
                        ProducedQuantity = BuildCycles * BuildItems.GetBuiltItemList(i).PortionSize
                        ExcessQuantity = ProducedQuantity - BuildItems.GetBuiltItemList(i).ItemQuantity
                    End If
                    ...
                    lstBuildItem.SubItems.Add(CStr(FormatNumber(BuildCycles, 0)))
                    lstBuildItem.SubItems.Add(CStr(FormatNumber(ProducedQuantity, 0)))
                    lstBuildItem.SubItems.Add(CStr(FormatNumber(ExcessQuantity, 0)))
                Next
            End If
        End If
